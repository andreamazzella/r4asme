...
subtitle: "R 4 ASME"
authors: Andrea Mazzella [(GitHub)](https://github.com/andreamazzella) & Lakmal
...
Get ondrate.dta in the same folder as this .Rmd notebook and import it.
...
Poisson regression in R uses the `glm()` formula – but it also needs to include `offset(log(<pyears>))` as a covariate. Clever statisticians, can you please explain what this means mathematically?
...
glm(disc2 ~ mfpermg + age + sex + offset(log(pyrs)),
    family = "poisson",
    data = ond) %>% idr.display()
```
...
You just need to use `as.integer()` with variable age. Unfortunately `idr.display()` breaks with quantiative exposures, so you need to extract the RR manually.
Also, *issue*, I don't know how to get the 95% CIs manually. There are two functions that are potentially relevant (`epiDisplay::ci.poisson()` and `survival::cipoisson()`, but I can't get them to work).
```{r}
# Age as numerical
...
# Get RR by exponentiating the log(RR)
exp(age_linear$coefficients)

# Not sure how to calculate 95% CI from the log(SE)?
summary(age_linear)
