...
subtitle: "R 4 ASME"
authors: Lakmal M. & Andrea Mazzella [(GitHub)](https://github.com/andreamazzella)
...
#-- You may prefer to create some intermediate object abd have a simpler Surv():
#trin$timein2 <- as.numeric(timein) / 365.25
#trin$timeout2 <- as.numeric(timeout) / 365.25
#trin$timebth2 <- as.numeric(timebth) / 365.25
#trin_surv_age2 <- trin %$% Surv(timein2, timeout2, death, origin = timebth2)
#-- Another way of calculating these years:
#trin$timein2 <- Epi::cal.yr(trin$timein, format="%d/%m/%Y")
...
# Factorise job grade
whitehall$grade <- factor(whitehall$grade,levels = c(1, 2), labels = c("higher", "lower"))
...
#whitehall$smok3 <- ifelse(whitehall$smok == 1, 1, ifelse(whitehall$smok == 2, 2,3)) %>% as.factor()
```
...
glm(chd ~ offset(log(pyears)) + grade + age,   # period goes here
  ...
  idr.display()
```
*Same issue* as in line 260, adj HR estimate is similar but not the same, and the age variable HR is completely off.

*Same workaround* Use the Epi package.

*Issue* code in line 415 and possibly further down is broken – I suspect this is bc you used the whitehall object that you had somehow changed in another Rmd?
...
# Lexis expansion of the whitehall datasex
